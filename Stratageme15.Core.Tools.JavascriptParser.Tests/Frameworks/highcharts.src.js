...
(function ($) {
	...
	win.HighchartsAdapter = win.HighchartsAdapter || ($ && {
		...
		animate: function (el, params, options) {
	        ...
	        if(options.delay) {
                $el.delay(options.delay).stop();
                $el.delay(options.delay).animate(params, options);
            } else {
                $el.stop();
...
function MouseTracker(chart, options) {
    ...
    //console.log(options.plotOptions.pie.cursor)
...
Point.prototype = {
	...
	onMouseOver: function () {
		...
		window.hoverPoint = point;// глобальный объект для кликабельности лейблов
	},
			...
			each(points, function (point) {
				...
				// in the point options, or if they fall outside the plot area.
					...
					// create new label
					    ...
					    //обработка клика по надписи
						if (point.dataLabels.distance < 0) {
						    dataLabel.css({ cursor: "pointer" });
					        dataLabel.element.onclick = function () {
					            if (window.hoverPoint) {
					                window.hoverPoint.firePointEvent('click');
					            }
					        };
					        
					        dataLabel.element.onmouseover = function () {
					            point.onMouseOver();
					        };
					    }
					}
...
var ColumnSeries = extendClass(Series, {
	...
	drawTracker: function () {
		...
		each(series.points, function (point) {
            // if this sector contains subdata - mark him by pointer cursor
            var attrdgm = "";
            if (point.isOpenable) {
                cssCursor = 'pointer';
                elemClass = 'hover1';
				...
				if (tracker) {// update
					...
					point.tracker =
                        ...
                        .on(hasTouch ? 'touchstart' : 'mouseover', function (event) {
							...
							point.onMouseOver();
                            if (attrdgm) {
                                $(".hovered").addClass("hovered_highlight");
                            }
						})
						.on('mouseout', function (event) {
                            ...
                            if (attrdgm) {
                                $(".hovered").removeClass("hovered_highlight");
                            }
						})
...
var PiePoint = extendClass(Point, {
	...
	init: function () {
		...
		var dgm = arguments[0];
	    var singleBockDiagram = dgm != null && dgm.yData.length == 1;
	    if (!singleBockDiagram) {
	        // add event listener for select
	        toggleSlice = function(e) {
	            if (singleBockDiagram) return;
	            (e.type === 'select') && !point.sliced && point.slice();
	            (e.type === 'unselect') && point.sliced && point.slice();
	        };
	...
	slice: function (sliced, redraw, animation) {
        ...
        point.group.animate(translation, {delay: sliced * 400});
        if (point.shadowGroup) {
            point.shadowGroup.animate(translation, {delay: sliced * 400});
        }
	}
});
...
var PieSeries = {
	...
	translate: function () {
		...
		series.getX = function (y, left, pnt) {
			var ld = labelDistance;
			if (pnt && pnt.dataLabels && pnt.dataLabels.distance) {
				ld = pnt.dataLabels.distance;
			}
			angle = math.asin((y - positions[1]) / (positions[2] / 2 + ld));

			return positions[0] +
				(left ? -1 : 1) *
				(mathCos(angle) * (positions[2] / 2 + ld));
		};
		...
		for (i = 0; i < len; i++) {
			...
			var dlPersist = labelDistance;
			if (point.dataLabels && point.dataLabels.distance){
				labelDistance = point.dataLabels.distance;
			}
			point.labelPos = [
	...
	drawDataLabels: function () {
		...
		while (i--) {
			...
			for (j = 0; j < length; j++) {
				...
				naturalY = labelPos[1];
				var pdl = point.dataLabels;
				var pointDlDistance = pdl ? (pdl.distance?pdl.distance:distanceOption):distanceOption;
				
				if (pointDlDistance > 0) {
					slot = usedSlots.pop();
					slotIndex = slot.i;
				...
				x = options.justify ? 
					...
					series.getX(slotIndex === 0 || slotIndex === slots.length - 1 ? naturalY : y, i,point);
				...
				// draw the connector
				if (outside && connectorWidth && pointDlDistance > 0) {
					connector = point.connector;
